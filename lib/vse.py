    ...
    def forward(self, images, captions, lengths, img_ids=None, warmup_alpha=1., ):
        ...
        if self.opt.multi_gpu:
            ...
            if max_len > cap_emb.shape[1]:
                # (B, L_max - L, C)
                pad_emb = torch.zeros(cap_emb.shape[0], max_len - cap_emb.shape[1], cap_emb.shape[2], ).to(
                    cap_emb.device)
        ...
        improved_sims, score_img_mask_all = self.forward_sim(img_emb, cap_emb, lengths)

        # basic alignment loss
        align_loss = self.criterion(img_emb, cap_emb, img_ids, improved_sims) * warmup_alpha

        # monitor
        monitor_loss = (score_img_mask_all.mean() - self.opt.img_sparse_ratio) ** 2

        loss = align_loss + self.opt.monitor_weight * monitor_loss
